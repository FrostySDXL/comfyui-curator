...
ComfyUI native extension
  ...
  ├── image_curator/native_routes.py  ← aiohttp batch/image foundation adapter
  ...
  (Move/favorites/publish/prompt-history/watcher/AI routes remain owned by app.py)
...
| | `image_curator/native_settings.py`, `image_curator/native_routes.py` | Resolve ComfyUI-owned paths and provide namespaced settings, batch/state/import, image, metadata, thumbnail, and original routes without Flask lifecycle imports |
...
- **Native extension scope:** Native settings and batch/image foundation routes are namespaced under `/api/curator/*` and `/curator/{thumb,image}/*`. Move/delete, favorites, publish, prompt history, watcher, and AI lifecycle remain standalone-only. See `COMFYUI_EXTENSION_PORT_SPEC.md`.
